...
st.session_state.setdefault("costops_plan_name", "Free" if NATIVE_APP_MODE else "Enterprise")
...
def executive_report_context(
    ...
    if report_type == "Savings by category" and not category_report.empty:
        ...
        largest_team_sentence = "No teams are staffed yet."
        if largest_team is not None:
            largest_team_sentence = (
                f"{largest_team['team']} is the largest staffed team with "
                f"{int(largest_team['assigned_users']):,} users."
            )
        narrative = (
